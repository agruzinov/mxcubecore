...
class P11Collect(AbstractCollect):
    ...
    def data_collection_hook(self):
        ...
        try:
            ...
            if collection_type == "Characterization":
                ...
                self.log.debug("Create diffraction snapshots...")
    ...
    def generate_xds_template(self):
        ...
        # FIXME Transfer to slurm job submissioin.
        # Prototype for the cluster. Change to slurm submission. Currently emulating CC work with eval.
        cmd_tpl = "\"cd '{processpath:s}'; " + \
                "/opt/xray/XDSAPP/3.1.9/generate_XDS.INP '{imagepath:s}'\" >/dev/null 2>&1\n"

        btHelper = triggerUtils.Trigger()
        ssh = btHelper.get_ssh_command()

        cmd = cmd_tpl.format(imagepath=h5file.replace("/gpfs","/beamline/p11"), processpath=process_dir.replace("/gpfs","/beamline/p11"))
        os.system("%s %s" %(ssh, cmd))
        print("%s %s" %(ssh, cmd))
