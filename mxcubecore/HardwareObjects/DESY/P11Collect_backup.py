...
class P11Collect(AbstractCollect):
    ...
    def add_h5_info(self):
        ...
        while not os.path.exists(h5file):
            if time.time() - start_wait > FILE_TIMEOUT:
                raise BaseException(
                    "Cannot add info to H5 file. Timeout waiting for file on diske"
                )
            time.sleep(0.5)
    ...
    def generate_xds_template(self):
        ...
        cmd_tpl = (
            "\"sleep 20; module load xdsapp/3.0.8; cd '{processpath:s}'; "
